...
registerController.generarSetUpClub = [
    ...
    async (request, response) => {
        try {
            ...
            if (usuario.length !== 0) {
                logger.warn('\t> generarSetUpClub: El usuario ya existe (registerController.js)');
...
// Eliminar cuenta
registerController.deleteUserAndClub = async (request, response) => {
    try {
        const { clubId } = request.body;

        logger.info('\t> deleteUserAndClub: club ID del usuario a eliminar: ' + clubId + ' (registerController.js)');
        if (!clubId) {
            logger.warn(`\t> deleteUserAndClub: El club con ID ${clubId} no existe (registerController.js)`);
            return response.status(404).json({ message: 'El club no existe' });
        }

        const result1 = await DJ.deleteMany({ club: new ObjectId(clubId) });
        logger.warn(`\t> deleteUserAndClub: ${result1.deletedCount} djs eliminados (registerController.js)`);
        
        const result2 = await Ingreso.deleteMany({ club: new ObjectId(clubId) });
        logger.warn(`\t> deleteUserAndClub: ${result2.deletedCount} ingresos eliminados (registerController.js)`);
        
        const result3 = await Mejora.deleteMany({ club: new ObjectId(clubId) });
        logger.warn(`\t> deleteUserAndClub: ${result3.deletedCount} mejoras eliminados (registerController.js)`);
        
        const result4 = await Producto.deleteMany({ club: new ObjectId(clubId) });
        logger.warn(`\t> deleteUserAndClub: ${result4.deletedCount} productos eliminados (registerController.js)`);
        
        const result5 = await Tecnico.deleteMany({ club: new ObjectId(clubId) });
        logger.warn(`\t> deleteUserAndClub: ${result5.deletedCount} tecnicos eliminados (registerController.js)`);

        const result6 = await Club.findByIdAndDelete(clubId);
        logger.warn(`\t> deleteUserAndClub: club eliminado (registerController.js)`);

        const result7 = await Usuario.deleteMany({ club: new ObjectId(clubId) });
        logger.warn(`\t> deleteUserAndClub: ${result7.deletedCount} club eliminado (registerController.js)`);

        logger.info('\t> deleteUserAndClub: Todas las colecciones eliminadas (registerController.js)');
        response.json('SUCCESS');

    } catch (error) {
        logger.error('Error: deleteUserAndClub: eliminar todas las colecciones (registerController.js): ' + error);
        response.status(500).json({ message: 'Error al eliminar todas las colecciones' });
    }
}
